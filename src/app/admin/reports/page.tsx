    ...
    <div className="h-full flex flex-col min-w-0 bg-slate-50/50">
      ...
      <div className="p-4 lg:p-8 shrink-0 bg-white border-b border-slate-100 lg:bg-transparent lg:border-0">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 font-bold text-sm">Real-time performance overview</p>
          </div>
          <div className="flex gap-2 bg-slate-100/50 p-1 rounded-xl lg:bg-white lg:p-1.5 lg:rounded-2xl shadow-sm border border-slate-100">
            {["today", "week", "month"].map(p => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={cn(
                  "px-4 lg:px-6 py-2 lg:py-2.5 rounded-lg lg:rounded-xl text-[10px] font-black uppercase tracking-widest transition-all",
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:px-8 space-y-10 pb-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((s, idx) => (
            <div key={idx} className="bg-white p-6 rounded-2xl lg:rounded-[2.5rem] border border-slate-100 shadow-sm">
              <div className={cn("w-12 h-12 rounded-xl lg:rounded-2xl flex items-center justify-center mb-4", s.bg)}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 pb-12">
          ...
          <div className="bg-white p-6 lg:p-8 rounded-2xl lg:rounded-[3rem] border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              <h4 className="text-lg font-black text-slate-900">Revenue Trend</h4>
          ...
          {/* Best Sellers */}
          <div className="bg-white p-6 lg:p-8 rounded-2xl lg:rounded-[3rem] border border-slate-100 shadow-sm">
            <h4 className="text-lg font-black text-slate-900 mb-8">Best Selling Items</h4>
              ...
              {data.bestSellers.length === 0 ? (
                ...
                data.bestSellers.map((item: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-slate-50 rounded-xl lg:rounded-2xl flex items-center justify-center font-black text-xs text-slate-400 shrink-0">
                      #{idx + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-black text-slate-900 text-sm truncate">{item.name}</p>
                      <div className="w-full bg-slate-50 h-2 rounded-full mt-2 overflow-hidden">
                        <div 
                    ...
                    <div className="text-right shrink-0">
                      <p className="font-black text-slate-900 text-sm">{item.sales} sold</p>
            ...
            {data.bestSellers.length > 0 && (
              <button className="w-full mt-8 py-4 bg-slate-50 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-100 transition-all flex items-center justify-center gap-2">
                View All Products
