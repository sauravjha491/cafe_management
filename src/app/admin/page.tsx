    ...
    <div className="h-full flex flex-col min-w-0 bg-slate-50/50">
      <div className="p-4 lg:p-8 shrink-0 bg-white border-b border-slate-100 lg:bg-transparent lg:border-0">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">Dashboard Overview</h1>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:px-8 space-y-10 pb-20">
          ...
          {cards.map((card, index) => {
            ...
            return (
              <motion.div
                ...
                key={card.label}
                className="bg-white p-6 rounded-2xl lg:rounded-[2.5rem] shadow-sm border border-slate-100 group hover:shadow-md transition-all"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className={cn("p-4 rounded-xl lg:rounded-2xl transition-transform group-hover:scale-110 duration-300", card.bg)}>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          {/* Recent Activity */}
          <div className="xl:col-span-2 bg-white p-6 lg:p-8 rounded-2xl lg:rounded-[3rem] shadow-sm border border-slate-100">
            ...
            <div className="space-y-4">
              {stats.recentOrders.length === 0 ? (
                <div className="py-10 text-center text-slate-400 font-bold">No orders yet today</div>
              ) : (
                stats.recentOrders.map((order) => (
                  <div key={order.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-slate-200 hover:bg-white transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-slate-900 tracking-tight">{order.customerName}</p>
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Table {order.tableNumber} • {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <span className={cn(
                          ...
                          order.status === "SERVED" ? "bg-slate-200 text-slate-600" : "bg-green-100 text-green-600"
                        )}>
                          {order.status}
                        </span>
                      </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 p-6 lg:p-8 rounded-2xl lg:rounded-[3rem] text-white shadow-xl shadow-slate-200">
              <h2 className="text-xl font-black mb-6 tracking-tight">Quick Actions</h2>
              <div className="grid grid-cols-1 gap-3">
                <Link href="/admin/menu" className="flex items-center gap-3 p-4 bg-white/10 rounded-xl lg:rounded-2xl hover:bg-white/20 transition-all group">
                ...
                <Link href="/admin/staff" className="flex items-center gap-3 p-4 bg-white/10 rounded-xl lg:rounded-2xl hover:bg-white/20 transition-all group">
                ...
                <Link href="/admin/settings" className="flex items-center gap-3 p-4 bg-white/10 rounded-xl lg:rounded-2xl hover:bg-white/20 transition-all group">
            ...
            <div className="bg-red-50 p-6 lg:p-8 rounded-2xl lg:rounded-[3rem] border border-red-100">
              <div className="flex items-center gap-3 mb-4">
                <TrendingUp className="w-6 h-6 text-red-600" />
                <h2 className="text-lg font-black text-slate-900 tracking-tight">Growth Tip</h2>
              </div>
